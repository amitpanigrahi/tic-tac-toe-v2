...
function initialize() {
    ...
    const wcParam = urlParams.get('wc');
    const wcText = document.getElementById('wc-tag');
    const unText = document.getElementById('player');
    if (wcParam) {
        wcText.innerText = wcParam;
    }
    else {
        wcText.innerText = "Tic-Tac-Toe";
    }
    if (unParam) {
        unText.innerText = unParam;
    }
    else {
        unText.innerText = "Player";
    }
    // debugger;
}
...
const resetScore = () => {
    score = {ties: 0, player: 0, computer: 0};
    updateScore();
}
...
function getOptions() {
    ...
    for (var i = 0; i < diffs.length; i++) {
        if (diffs[i].checked) {
            if (i) {
                document.getElementById("mode-s").innerText = "Hard"
            }
            else {
                document.getElementById("mode-s").innerText = "Easy"   
            }
            if (difficulty !== i) {
                resetScore();
            }
